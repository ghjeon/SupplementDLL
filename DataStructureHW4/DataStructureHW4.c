...
void swap(Product_pointer node1, Product_pointer node2);
Product_pointer sortBycount(Product_pointer target);
Product_pointer sortByno(Product_pointer target);
int _p_countNodes();
...
{//insert product node in product's last
	return _p_InsertNodeRight(product->prev, node);//insert node
}
...
void sortList(Product_pointer target)
{
	int sortCrit;

	if ((_p_countNodes() == 0) | (_p_countNodes() == 1)){ // 멤버가 한개이면 정렬할 필요 없음.
		_p_readAll();
		return;
	}

	do{  // 1 또는 2가 입력되지 않으면 재입력
		printf("무엇을 기준으로 정렬하시겠습니까(1: 넘버, 2: 수량, 3: 메뉴로 이동)? => ");
		scanf_s("%d", &sortCrit,sizeof(sortCrit));
		fflush(stdin);
		if ((sortCrit != 1) & (sortCrit != 2) & (sortCrit != 3)) printf("1 또는 2 또는 3을 입력하여야 합니다.\n");
...
Product_pointer sortBycount(Product_pointer target)
{
	Product_pointer pointer = target->next;
	Product_pointer prev_pointer = target->next; // 항상 pointer 변수가 가리키는 멤버의 앞을 가리킴. pointer가 target다음일 때에는 prev_pointer도 target 다음을 가리킴.
	Product_pointer temp_pointer;
	int i = 0, j = 0;

	for (i = _p_countNodes() - 1; i > 0; i--)
	{
		for (j = 0; j < i; j++)
		{
			if (pointer->count > pointer->next->count)
			{
				if (pointer == target->next) // 리스트의 첫 노드와 다음 노드를 바꿀 때
				{  
					target->next = pointer->next;
					temp_pointer = target->next->next;
					target->next->next = pointer;
					pointer->next = temp_pointer;
					pointer = target->next;
					prev_pointer = target->next;
				}
				else
				{ 
					temp_pointer = pointer->next;
					pointer->next = temp_pointer->next;
		...
		pointer = target->next; //처음으로 되돌아감.
		prev_pointer = pointer;
	}

	_p_readAll(); // 수정된 리스트 출력
	return target;
}
...
{
	Product_pointer pointer = target->next;
	Product_pointer prev_pointer = target->next; // 항상 pointer 변수가 가르키는 멤버의 앞을 가리킴. pointer가 target와 같은때는 똑같이 target임.
	Product_pointer temp_pointer;
	int i = 0;
	int j = 0;
